...
/// This macro implements FromBits for the portable and the architecture specific vector types
///
...
macro_rules! impl_arch {
    ...
    ($arch_head:ident | $($from_ty:ident),* | $($into_ty:ident),*) => {
        impl_from_bits!($arch_head: $($from_ty),*);
        impl_into_bits!($arch_head: $($into_ty),*);
    };
}
